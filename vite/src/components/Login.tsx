...
const Login = ({setShowLogin}:props) => {
  ...
  return (
    <div onClick={() => setShowLogin(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex justify-center items-center text-sm text-gray-600 bg-black/50'>
    {/* adding form cmpnent */}
    <form onSubmit={handleSubmit} onClick={(e)=>e.stopPropagation()} className="sm:w-[350px] w-full text-center border border-gray-300/60 rounded-2xl px-8 bg-white">
                ...
                <h1 className="text-primary text-3xl mt-10 font-medium">{state === "login" ? "Login" : "Sign up"}</h1>
